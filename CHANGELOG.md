...
- **BREAKING: `MenuItem::getLink()` returns `?string`** instead of `string`. Items that don't produce a link — `no-link`, `breakpoint`, a `page` / `file` type with no relation set, or a `url` type with an empty `Url` — now return `null` rather than the empty-string sentinel. The `updateLink` extension hook signature changes to `function updateLink(?string &$link)`; downstream extensions that compared against `''` should compare against `null`. Templates that gate on `<% if $Link %>` are unaffected
- **`MenuItem::getLink()` is now a `match` over `LinkType`.** The function is exhaustive against the enum and no longer needs the trailing `?? ''` fallback
